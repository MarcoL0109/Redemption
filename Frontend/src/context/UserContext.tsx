...
interface UserContextType {
    userData: UserData; // 💡 Keep as UserData so it gracefully drops back to DEFAULT_USER
    loading: boolean;
    logout: () => void;
...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchUser = useCallback(async () => {
        setLoading(true);
        try {
            const getSessionInfoResponse = await fetch(`${API_ROUTES.UTILS}/SessionInfo`, {
                method: "GET",
                credentials: "include"
            });

            if (getSessionInfoResponse.status === 200) {
                const session_info_body = await getSessionInfoResponse.json();
